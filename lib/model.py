...
class WideSEBottleneck(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None,
                 *, reduction=16):
        super(WideSEBottleneck, self).__init__()
...
def se_resnext101_32x48d(num_classes=1000):
    """Constructs a ResNeXt-101 32x32 model
    """
    model = ResNet(SEBottleneck, [3, 4, 23, 3], groups=32, width_per_group=48, num_classes=num_classes)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model


def wide_se_resnext101_32x32d(num_classes=1000):
    """Constructs a ResNeXt-101 32x32 model
    """
    model = ResNet(WideSEBottleneck, [3, 4, 23, 3], groups=32, width_per_group=32, num_classes=num_classes)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model
